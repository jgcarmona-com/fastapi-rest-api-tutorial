...
def mock_get_password_hash():
    with patch("qna_api.auth.services.get_password_hash", autospec=True) as mock_service:
        yield mock_service

@pytest.fixture
def mock_create_user_service():
    with patch("qna_api.auth.services.create_user_service", autospec=True) as mock_service:
        yield mock_service
...
def test_create_user(test_client: TestClient, mock_get_password_hash: MagicMock, mock_create_user_service: MagicMock):
    ...
    mock_create_user_service.return_value = mock_user
...
def test_login_for_access_token(test_client: TestClient, mock_db: MagicMock):
    ...
    with patch("qna_api.auth.services.verify_password", return_value=True):
        with patch("qna_api.auth.services.create_access_token", return_value="testtoken"):
            response = test_client.post("/auth/token", data={"username": "testuser", "password": "password123"})
            print(response.text)  # Agregar impresión para depuración
            assert response.status_code == 200
            data = response.json()
