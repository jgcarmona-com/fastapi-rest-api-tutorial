...
class AnswerController:
    ...
    async def create_answer(self, answer: AnswerCreate, db: Session = Depends(get_db), current_user: User = Depends(get_authenticated_user)):
        logger.info(f"{current_user.full_name} is creating an answer")
        return self.answer_service.create_answer(answer)

    async def get_answer(self, answer_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_authenticated_user)):
    ...
    async def update_answer(self, answer_id: int, answer: AnswerCreate, db: Session = Depends(get_db), current_user: User = Depends(get_authenticated_user)):
    ...
    async def delete_answer(self, answer_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_authenticated_user)):
